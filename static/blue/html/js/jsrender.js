...
//		}
	...
	function parseParams(params, pathBindings, tmpl) {
		...
		//   lftPrn0        lftPrn        bound            path    operator err                                                eq             path2       prn    comma   lftPrn2   apos quot      rtPrn rtPrnDot                    prn2  space
			...
			if (bound && !eq) {
				path = bound + path; // e.g. some.fn(...)^some.path - so here path is "http://yuqing.gsdata.cn/themes/blue/html/js/^some.path"
			}
			operator = operator || "";
			...
			function parsePath(allPath, not, object, helper, view, viewProperty, pathTokens, leafToken) {
				...
				if (object) {
					...
					if (!subPath) {
						...
						allPath = not + (allPath.slice(0, 9) === "http://yuqing.gsdata.cn/themes/blue/html/js/view.data"
	...
	function buildCode(ast, tmpl, isLinkExpr) {
		...
		code = "// " + tmplName
			...
			+ (hasEncoder ? "http://yuqing.gsdata.cn/themes/blue/html/js/,h=j.converters.html" : "") // html converter
			+ (isLinkExpr ? ";\n" : ',ret=""\n')
